...
출력 형식 규칙 (반드시 준수):
- 각 항목은 반드시 새로운 줄에 출력하세요.
- 항목 내용이 길어도 줄바꿈 없이 한 줄로 쓰지 말고, 아래 형식을 그대로 유지하세요.
- 의견과 의견 사이에는 반드시 구분선(---)을 넣으세요.

각 심의의견마다 아래 형식으로 출력하세요 (각 항목은 반드시 별도 줄에):
...
📋 **의견 원문**:
(메일에서 추출한 심의의견 원문 그대로)

📄 **페이지 번호**:
(소재 파일의 [슬라이드 N] 번호 기준, 여러 개면 전부 나열)

🏷️ **배너 번호**:
(슬라이드 내 '(1)배너' '(2)배너' 등 텍스트 기준, 없으면 '해당 없음')

❌ **현재 문구**:
(소재에서 찾은 현재 문구 또는 이미지 설명)
...
🔄 **수정 문구**:
(해당 페이지/배너에서 실제로 찾은 문구를 수정 파일에서 그대로 복사해서 붙여넣기. 요약하거나 바꾸지 말고 원문 그대로.)

✅ **확인 내용**:
(위 수정 문구가 심의의견의 요구사항을 반영하고 있는지 코멘트)
