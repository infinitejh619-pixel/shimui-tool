...
with tab1:
    ...
    if st.button("1:1 매칭 분석하기", type="primary", key="btn_match"):
        if not email_text.strip():
            ...
            msg = "이미지 OCR 포함 분석 중... (잠시 기다려주세요)" if use_ocr else "소재 분석 중..."
            with st.spinner(msg):
                try:
...
3. 소재(텍스트 및 이미지 OCR)에서 해당 문구를 찾습니다.
4. 정확한 수정 방향을 제시합니다.
...
- 📄 페이지 번호: N페이지 (원문 표기 병기, 예: 원문 2,7p → 2페이지와 7페이지)
- 🏷️ 배너 번호: N번 배너 (배너 언급 없으면 '없음')
- ❌ 현재 문구: (소재에서 찾은 실제 문구 — 위치가 여러 곳이면 각각 표시)
- ✅ 수정 문구: (바꿔야 할 문구)
- 💡 수정 이유: (한 줄 요약)
---

주의: 의견 원문을 절대 요약하거나 변형하지 마세요.

===== 심의 메일 =====
{email_text}

===== 소재 내용 (페이지별, 이미지 OCR 포함) =====
{file_text}"""
...
with tab3:
    ...
    with col_left:
        st.markdown("#### 신규 매체/지면 등록")
        new_media     = st.text_input("매체명", placeholder="예: 카카오")
        new_placement = st.text_input("지면",   placeholder="예: 배너")
        new_category  = st.text_input("구분",   placeholder="예: 광고문구 1")
        new_notes     = st.text_input("비고",   placeholder="예: 윗배너")
        new_limit     = st.number_input("글자수 제한", min_value=1, value=20, step=1)
        new_spaces    = st.checkbox("공백 포함 글자수", value=True)
        ...
        if st.button("➕ 등록", use_container_width=True):
            if new_media and new_placement and new_category:
                media_settings.append({
                    ...
                    "notes": new_notes,
                    "limit": int(new_limit),
                    "include_spaces": new_spaces,
                })
                save_media(media_settings)
                st.success("등록 완료!")
                st.rerun()
            else:
                st.error("매체명, 지면, 구분은 필수 입력이에요.")
        ...
        for idx, s in enumerate(media_settings):
            ...
            with col_a:
                st.markdown(
                    f"**{s['media']}** — {s['placement']} — {s.get('category','')}{notes_text}  \n"
                    f"`{s['limit']}자` ({space_label})"
                )
            with col_b:
    ...
    with col_right:
        ...
        if not media_settings:
            ...
            def option_label(s):
                ...
                space = "공백포함" if s["include_spaces"] else "공백제외"
                return f"{s['media']} — {s['placement']} — {s.get('category','')}{notes}  [{s['limit']}자/{space}]"

            selected_idx = st.selectbox("매체/지면 선택", range(len(media_settings)),
                                        format_func=lambda i: option_label(media_settings[i]))
            setting = media_settings[selected_idx]

            if setting.get("notes"):
                st.caption(f"💡 {setting['notes']}")

            check_text = st.text_area("문구 입력", height=200,
            ...
            if check_text:
                ...
                st.caption(
                    f"공백 포함: {len(check_text)}자 · "
                    f"공백 제외: {len(check_text.replace(' ',''))}자"
                )
