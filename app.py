...
def extract_image_text(image_bytes, slide_num, image_num):
    """이미지 안의 텍스트를 Groq Vision 모델로 추출"""
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[{
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"이 이미지는 보험 광고 소재({slide_num}페이지, {image_num}번째 이미지)입니다. 이미지 안에 보이는 모든 텍스트를 빠짐없이 추출해주세요. 텍스트만 출력하고 다른 설명은 하지 마세요."
                    },
                    {
                        "type": "image_url",
    ...
    if uploaded_file.name.lower().endswith(".pdf"):
        ...
        for i, slide in enumerate(prs.slides, 1):
            ...
            for shape in slide.shapes:
                ...
                # 이미지에서 OCR 추출 (shape_type 13 = PICTURE)
                if shape.shape_type == 13:
                    image_num += 1
                    try:
                        ocr = extract_image_text(shape.image.blob, i, image_num)
                        if ocr:
                            slide_text += f"[{image_num}번 이미지 내 텍스트]\n{ocr}\n"
                    except Exception:
                        pass
...
with tab1:
    ...
    st.caption("심의 메일과 소재 파일을 함께 넣으면, 이미지 안 텍스트까지 읽어서 각 의견을 정확히 매칭해드려요.")
    ...
    if st.button("1:1 매칭 분석하기", type="primary", key="btn_match"):
        if not email_text.strip():
            ...
            with st.spinner("이미지 포함 소재 전체를 분석하는 중... (이미지가 많으면 1~2분 걸릴 수 있어요)"):
                try:
...
소재 내용에는 이미지 OCR 결과도 포함되어 있으니 함께 참고하세요.

작업 방법:
1. 심의 메일에서 각 심의의견을 찾아 원문 그대로 인용합니다.
2. 아래 위치 표기 파싱 규칙을 따라 해당 페이지와 배너를 정확히 파악합니다.
3. 해당하는 모든 페이지/배너의 문구를 소재(텍스트 및 이미지 OCR)에서 찾습니다.
...
- 📄 해당 위치: N페이지 / N번 배너 (원문 위치 표기 병기)
- ❌ 현재 문구: (소재 텍스트 또는 이미지 OCR에서 찾은 실제 문구)
- ✅ 수정 문구: (바꿔야 할 문구)
- 💡 수정 이유: (한 줄 요약)
---
    ...
    if not selected:
        ...
        if revised_file:
            ...
            if st.button("검수 시작하기", type="primary", key="btn_check"):
                with st.spinner("이미지 포함 수정 소재를 검수하는 중..."):
                    try:
...
원본 심의의견 분석 결과와 수정된 소재(이미지 OCR 포함)를 비교하여 각 심의의견이 반영됐는지 검수해주세요.
...
【위치 파싱 규칙】
- "2,7p" → 2페이지와 7페이지 모두 확인
- "[2,7p_1,6배너]" → 해당 페이지의 해당 배너 모두 확인
